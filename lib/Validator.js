...
class Validator {
   ...
   * @param {RegExp} [params.allowed] - if set, an array of allowed values (more convenient than using a regex), or a function that will return such an array
  ...
  /**
   * Return the allowed values since the allowed passed to the validator can be a function
   * @returns {Array<string|number>|null} - return the allowed values or null
   */
  _getAllowed() {
    return ( typeof this.allowed == 'function' ) ? this.allowed() : this.allowed;
  }
  ...
  _createMessage(errors) {
    ...
    if ( errors[Test.allowed] ) m.push(FailMessages.allowed.replace('##',this._getAllowed().join(',')))
  ...
  validate(value) {
    ...
    if ( value == null ) {
      ...
      let allowed = this._getAllowed();
      if ( allowed && Array.isArray(allowed) && value != null && value != '' && !allowed.includes(value) ) {
        errors[Test.allowed] = allowed;
      }
    }
